...
func seedIfNeededWithRoles(roleAdmin entity.Role, roleUser entity.Role) {
	...
	ug1 := entity.UserGame{Status: "active", GrantedAt: time.Now().Add(-48 * time.Hour), GameID: g1.ID, UserID: u1.ID}
	ug2 := entity.UserGame{Status: "active", GrantedAt: time.Now().Add(-24 * time.Hour), GameID: g2.ID, UserID: u1.ID}
	db.Create(&ug1)
	db.Create(&ug2)

	// กระทู้/คอมเมนต์/รีแอคชัน/ไฟล์/แจ้งเตือน
	th := entity.Thread{Title: "รวมทริคมือใหม่ Space Odyssey", Content: "แชร์ทริคและคำถามได้ที่คอมเมนต์เลยครับ", UserID: u1.ID, GameID: g1.ID}
	db.Create(&th)

	cm1 := entity.Comment{Content: "โหมด co-op เล่นยังไงให้ผ่านด่าน 3 ดีครับ", UserID: u2.ID, ThreadID: th.ID}
	db.Create(&cm1)

	rc := entity.Reaction{TargetType: "thread", TargetID: th.ID, Type: "like", UserID: u2.ID}
	db.Create(&rc)

	at := entity.Attachment{TargetType: "comment", TargetID: cm1.ID, FileURL: "https://example.com/tips.png", UserID: u2.ID}
	db.Create(&at)

	noti := entity.Notification{Title: "มีคอมเมนต์ใหม่ในกระทู้ของคุณ", Type: "system", Message: "Bob ตอบในหัวข้อ: รวมทริคมือใหม่ Space Odyssey", UserID: u1.ID}
	db.Create(&noti)
}
